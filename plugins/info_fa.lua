...
local function res_user_callback(extra, success, result) -- /me <username> function
  if success == 1 then  
   ...
   Username = 'ندارد'
    ...
    if not value then
	 if result.id == tonumber(Arian) then
	   text = text..'مقام : سازنده و مدیر کل ربات \n\n'
...
local function action_by_id(extra, success, result)  -- /me <ID> function
 if success == 1 then
 if result.username then
   Username = '@'..result.username
   else
...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'me' and not matches[2] then
  local receiver = get_receiver(msg)
  local Reply = msg.reply_id
  if msg.reply_id then
  ...
  if matches[1]:lower() == 'me' and matches[2] then
